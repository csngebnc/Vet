...
export class BookAppointmentByDoctorComponent implements OnInit {
  ...
  valuechange(value) {
    if (value.trim().length > 0) {
      this.userService.getUserByEmail(value).subscribe((user: VetUserDto) => {
        if (user) {
          this.selectUserForm.patchValue({ userId: user.id })
          this.user = user;
        } else {
          this.selectUserForm.patchValue({ userId: '' })
        }
      });
    }
  }
  ...
  loadAnimals() {
    this.animalService.getUserAnimalsByUserId(this.user.id).subscribe((animals: AnimalDto[]) => {
      this.selectedOwnerName = this.user.realName;
